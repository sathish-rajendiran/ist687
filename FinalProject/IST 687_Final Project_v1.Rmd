...
# unique(map.world_olympics$region)
# colnames(map.world_olympics)

map.world_olympics <- map.world_olympics %>% mutate(fill_flg = ifelse(fill_flg=="TRUE", region, fill_flg))

# colnames(map.world_olympics)
# unique(map.world_olympics$fill_flg)
...
mapG <- ggplot() + geom_polygon(data = map.world_olympics, aes(x = long, y = lat, group = group,fill=Countries),show.legend = TRUE)+ scale_fill_manual(
    values = pal,
    limits = names(pal)
  )
mapG <- mapG +  
